...
def _fila(m: MovimientoCuenta, saldo: float) -> dict:
    return {
        ...
        "monto_origen": round(_f(m.monto_origen), 2) if m.monto_origen is not None else None,
        "moneda_origen": m.moneda_origen,
        "tasa_cambio": round(_f(m.tasa_cambio), 4) if m.tasa_cambio is not None else None,
        "saldo": round(saldo, 2),
        "nota": m.nota,
